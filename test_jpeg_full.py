...
#   test_jpeg_full.run_deep_pipeline(q=80, pool_depth=8)  # 深度緩衝管線
#   test_jpeg_full.run_deep_compare(q=80)  # 比較不同緩衝深度 vs FPS 穩定性
...
class FpsTracker:
    ...
    def record(self, decode_us, dma_us, total_us):
        self._decode.append(decode_us)
        self._dma.append(dma_us)
        self._total.append(total_us)

    @staticmethod
    def _summary(data):
        if not data:
            return (0, 0, 0, 0)
        return (min(data), max(data), sum(data) // len(data), max(data) - min(data))

    def report(self, label=""):
        n = len(self._total)
        if n == 0:
            return float('inf')
        d_min, d_max, d_avg, d_spread = self._summary(self._decode)
        m_min, m_max, m_avg, m_spread = self._summary(self._dma)
        t_min, t_max, t_avg, t_spread = self._summary(self._total)
        fps_avg = 1e6 / t_avg if t_avg else 0
        fps_min = 1e6 / t_max if t_max else 0
        fps_max = 1e6 / t_min if t_min else 0
        jitter_pct = (fps_max - fps_min) / fps_avg * 100 if fps_avg else 0
        prefix = label + ": " if label else ""
        print("{}n={} | dec spread={}us | dma spread={}us | frame spread={}us".format(
            prefix, n, d_spread, m_spread, t_spread))
        print("{}FPS: avg={:.1f} min={:.1f} max={:.1f} jitter={:.1f}%".format(
            prefix, fps_avg, fps_min, fps_max, jitter_pct))
        return jitter_pct
...
class PipelinePlayer:
    ...
    def _dma_fire(self, fb=None):
        """發送整幀 → 直接傳 fb memoryview 切片，零 copy。"""
...
class DeepBufferPipeline:
    def __init__(self, bus, dc, w, h, bpp, jpeg_frames,
        ...
        self._frame_budget_us = int(1e6 / target_fps) if target_fps > 0 else 0

        print("  allocating {} framebuffers ({}KB each)...".format(
            pool_depth, self._fb_size // 1024))
        ...
        for i in range(pool_depth):
            fb = _alloc_fb(self._fb_size)
            if fb is not None:
                self._bufs.append(fb)
                self._free.append(len(self._bufs) - 1)
            else:
                fail_count += 1

        actual_depth = len(self._bufs)
        if actual_depth < 2:
            raise MemoryError("DeepBufferPipeline: need >=2 framebuffers")
        if fail_count:
            print("  {} alloc(s) failed, pool depth={}".format(fail_count, actual_depth))
        ...
        self._frame_idx = 0
        self._frame_total = len(jpeg_frames)
        ...
        self._pool_empty_events = 0

        prefill_target = max(1, int(actual_depth * prefill_pct))
        self._prefill(prefill_target)

    def _advance_frame(self):
        self._frame_idx += 1
        if self._frame_idx >= self._frame_total:
            if self._loop:
                self._frame_idx = 0
                return True
            return False
    ...
    def _prefill(self, target):
        for _ in range(target):
            if not self._free:
                break
            idx = self._free.pop()
            self._dec.decode_into(self._frames[self._frame_idx], self._bufs[idx])
    ...
    def run(self, max_frames=0):
        self._running = True
        print("pipeline: {} frames | pool={} | {}x{} | fps={}".format(
            self._frame_total, self._pool_depth, self._w, self._h,
            self._target_fps if self._target_fps else "unlimited"))

        t_total = time.ticks_ms()
        try:
            while self._running:
                if max_frames and self._count >= max_frames:
                    break

                t_frame_start = time.ticks_us()

                if self._ready:
                    send_idx = self._ready.pop(0)
                else:
                    self._pool_empty_events += 1
                    if not self._free:
                        for tid in self._pending_tids:
                            self._bus.wait(tid)
                        self._pending_tids = []
                        self._free.append(self._sending_idx)
                    send_idx = self._free.pop()
                    self._dec.decode_into(
                        self._frames[self._frame_idx], self._bufs[send_idx])
                    self._advance_frame()
                ...
                t_fire = time.ticks_diff(time.ticks_us(), t0)

                t_decode = 0
                if self._free:
                    dec_idx = self._free.pop()
                    t0 = time.ticks_us()
                    self._dec.decode_into(
                        self._frames[self._frame_idx], self._bufs[dec_idx])
                    t_decode = time.ticks_diff(time.ticks_us(), t0)
                    self._ready.append(dec_idx)
                    self._advance_frame()
                ...
                self._free.append(send_idx)
                self._sending_idx = -1

                t_total_frame = time.ticks_diff(time.ticks_us(), t_frame_start)
                t_dma_effective = t_fire + t_wait

                if self._frame_budget_us > 0 and t_total_frame < self._frame_budget_us:
                    time.sleep_us(self._frame_budget_us - t_total_frame)
                    t_total_frame = self._frame_budget_us

                self._tracker.record(t_decode, t_dma_effective, t_total_frame)
                self._count += 1

                if self._count % 30 == 0:
                    dt = time.ticks_diff(time.ticks_ms(), t_total)
                    fps = self._count * 1000.0 / dt if dt else 0
                    empty_flag = " E{}".format(self._pool_empty_events) if self._pool_empty_events else ""
                    print("  frame={} fps={:.1f} | pool={}/{} | d={}us m={}us t={}us{}".format(
                        self._count, fps,
                        len(self._ready), self._pool_depth,
                        t_decode, t_dma_effective, t_total_frame, empty_flag))
            ...
            dt = time.ticks_diff(time.ticks_ms(), t_total)
            fps = self._count * 1000.0 / dt if dt else 0
            print("done: {} frames in {:.1f}s | {:.1f} fps".format(
                self._count, dt / 1000.0, fps))
            print("  buffer underruns: {}".format(self._pool_empty_events))
            self._tracker.report("stability")
...
def _gen_frames_pool(n, quality):
    frames = []
    for i in range(n):
        jb = _encode_noise(_W, _H, quality)
        frames.append(jb)
    sz = sum(len(f) for f in frames)
    print("  {} JPEG frames, {}KB total".format(n, sz // 1024))
    return frames


def run_deep_pipeline(quality=80, pool_depth=8, target_fps=0,
                      n_frames=20, max_display=200):
    if jpeg is None: print("jpeg not found"); return
    if lcd_bus is None: print("lcd_bus not found"); return
    from machine import Pin
    ...
    try:
        ...
        for name, caps in [
            ...
            print("  {}: free={}KB total={}KB largest={}KB".format(
                name, free // 1024, total // 1024, largest // 1024))
        pool_mem = pool_depth * fb_size
        print("  planned pool: {}x{}KB = {}KB".format(
            pool_depth, fb_size // 1024, pool_mem // 1024))
        if pool_mem > heap_caps.get_largest_free_block(heap_caps.CAP_SPIRAM):
            print("  WARNING: pool may not fit in largest SPIRAM block")
    except Exception as e:
        print("  heap_caps unavailable: {}".format(e))
    ...
    gc.collect()
    frames = _gen_frames_pool(n_frames, quality)
...
def run_deep_compare(quality=80, n_frames=20, max_display=100):
    if jpeg is None: print("jpeg not found"); return
    if lcd_bus is None: print("lcd_bus not found"); return
    from machine import Pin

    print("\n" + "=" * 72)
    print("DEEP BUFFER COMPARISON: pool depth vs FPS stability")
    print("=" * 72)
    ...
    print("\n" + "=" * 72)
    print("Comparison complete — check 'buffer underruns' above")
    print("  lower underruns + lower jitter% = more stable FPS")
    print("=" * 72)
